...
function FormItem({name, label, type}: {name:string, label:string, type:string}){
  return (
    <div className={`flex flex-col w-full md:w-1/2 pr-3`}>
      ...
      <input type={type} name={name} id={name} className='px-3 py-2 text-zinc-700 shadow-lg text-sm' />
    </div>
  )
}
...
export default function ContactUs() {
  ...
  return (
    <section className="flex bg-gradient-to-t from-zinc-300 to-zinc-100">
      <div className="md:w-1/2 p-10 md:pl-[15%] md:py-16 md:pr-10">
        <h2 className='border-l-8 border-orange-600 pl-3 mb-5'>CONTACTANOS</h2>
        <form className="flex flex-wrap gap-y-3" onSubmit={handleSubmit} id='contact-form'>
          ...
          <div className='flex flex-col w-full md:w-1/2 pr-3'>
            ...
            <select name="reason" id="reason" className='p-2 text-zinc-700 shadow-lg rounded-none text-sm' placeholder='general'>
          ...
          <div className={`flex flex-col w-full h-24 pr-3`}>
            ...
            <textarea name='msg' id='msg' className='px-3 py-2 text-zinc-700 shadow-lg text-sm flex-grow line-clamp-3' />
          </div>
